...
/**
 * @ClassName: CompressUploadHandler
 * @Description: 数据压缩上传0x0901
 * @Company iot
 * @author
 * @version 1.0, 2017-3-28 上午10:11:15
 */
//@Component(value = "p_compress_upload")
public class CompressUploadHandler extends AbstractDeviceCommandHandler implements MessageID {
	...
	public Map<String, Object> receive(JT808ProtocalPack message, Channel tsession) throws Exception {
		...
		try {
			int messageLength = HexUtils.parseIntFromBytes(message.getBody(), 0, 4);
			byte[] messageContent = HexUtils.copyOfRange(message.getBody(), 1, message.getBody().length);
			Log.getLogger(this.getClass())
					.info("收到压缩信息，messageLength:" + messageLength + "messageContent:" + HexUtils.bytesToHexString(messageContent));
